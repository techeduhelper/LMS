...
export const getAllStudents = async (req, res, next) => {
    try {
        ...
        res.status(400).json({ message: `error in getting all students", ${err.message}` });
    }
};
...
export const addStudent = async (req, res, next) => {
    try {
        ...
        const { name, email, year, fatherName, aadharCard, gender, department, section, dob, studentMobileNumber, fatherMobileNumber } = req.body;
...
export const addFaculty = async (req, res, next) => {
    try {
        ...
        const { name, email, designation, department, facultyMobileNumber, aadharCard, dob, gender } = req.body;
